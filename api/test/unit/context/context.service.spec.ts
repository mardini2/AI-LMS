...
describe('ContextService', () => {
  ...
  beforeEach(() => {
    ...
    moodle = new MoodleClient(config as unknown as ConfigService);
    service = new ContextService(moodle, cache as unknown as Cache);

    // Moodle transport moved to MoodleClient — spy there, not on ContextService.
    callMoodleApi = jest.spyOn(moodle, 'callMoodleApi');
...
/** Emitted under every heading so the model does not read resource numbers as week numbers. */
...
describe('formatDocument', () => {
  it('formats a full document with heading, present metadata fields, and body', () => {
    ...
    expect(result).toBe(
      [
        '### Course section: Week 1 / Lecture Notes',
        SECTION_NUMBER_NOTE,
        `Metadata: type=resource_pdf; course=Organic Chemistry; course_section=Week 1; module=Lecture Notes; file=bonds.pdf; source=https://moodle.example/pluginfile.php/1; updated=${new Date(1_700_000_000 * 1000).toISOString()}`,
  ...
  it('formats a minimal document with only present metadata fields', () => {
    ...
    expect(result).toBe(
      [
        ...
        SECTION_NUMBER_NOTE,
        'Metadata: type=course_summary; course_section=Course',
...
describe('ContextService HTTP transport', () => {
  ...
  function createService(
    ...
    moodle = new MoodleClient(config as unknown as ConfigService);
    return new ContextService(moodle, cache as unknown as Cache);
  }

  /** callMoodleApi / downloadMoodleFile / normalizeMoodleFileUrl now live on MoodleClient. */
  function transport(): MoodleClient {
    return moodle;
  }
...
describe('ContextService.fetchFileDocument', () => {
  ...
  beforeEach(() => {
    ...
    const moodle = new MoodleClient(config as unknown as ConfigService);
    service = new ContextService(moodle, cache as unknown as Cache);

    downloadMoodleFile = jest.spyOn(moodle, 'downloadMoodleFile');
