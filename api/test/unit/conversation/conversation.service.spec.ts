...
import { Message, MessageRole } from '../../../src/conversation/entities/message.entity';
...
describe('ConversationService', () => {
  ...
  function createQueryBuilderMock<T extends { createdAt?: Date }>(
    ...
    // Stands in for MAX(createdAt) GROUP BY conversationId.
    qb.getRawMany = jest.fn(async () => {
      const latest = new Map<string, Date>();
      for (const row of rows as Array<T & { conversationId?: string }>) {
        if (!row.conversationId || !row.createdAt) continue;
        const current = latest.get(row.conversationId);
        if (!current || row.createdAt.getTime() > current.getTime()) {
          latest.set(row.conversationId, row.createdAt);
        }
      }
      return [...latest.entries()].map(
        ([conversationId, lastUserMessageAt]) => ({
          conversationId,
          lastUserMessageAt,
        }),
      );
  ...
  beforeEach(() => {
    ...
    conversationRepo = {
      ...
      update: jest.fn(
        async (criteria: { id: string }, partial: Partial<Conversation>) => {
          const idx = conversations.findIndex((c) => c.id === criteria.id);
          if (idx >= 0) {
    ...
    messageRepo = {
      ...
      count: jest.fn(
        async ({
          ...
          where: { conversationId?: string; role?: MessageRole };
        }) =>
          messages.filter(
            (m) =>
      ...
      delete: jest.fn(async (criteria: { conversationId?: string; id?: string }) => {
        const before = messages.length;
        messages = messages.filter((m) =>
          criteria.conversationId !== undefined
            ? m.conversationId !== criteria.conversationId
            : m.id !== criteria.id,
        );
        return { affected: before - messages.length };
      }),
      createQueryBuilder: jest.fn(() =>
        createQueryBuilderMock<Message>(
          ...
          (rows, _alias, condition, params) => {
            ...
            // Aggregate path filters on the camelCase property with an id list.
            if (condition.includes('conversationId IN') && Array.isArray(params.ids)) {
              const ids = params.ids as string[];
              filtered = filtered.filter((m) => ids.includes(m.conversationId));
            }
    ...
    it('maps results through toSummary', async () => {
      ...
      expect(summaries[0]).toEqual(
        expect.objectContaining({
          ...
          type: 'general',
          title: 'Main',
          // General chats tag off the display title, so casing follows 'Main'/'Home'.
          tag: '#Main',
          pinned: true,
          createdAt,
          updatedAt,
