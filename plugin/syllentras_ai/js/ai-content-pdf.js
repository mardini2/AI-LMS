/**
 * Shared AI Content PDF export (study guide / flashcards / practice quiz).
 * Loaded from before_footer.php — used by chat panel and on-page manage bar.
 *
 * window.SyllentrasAiContentPdf.download(item, config) -> Promise
 *   item:   { cmId, kind?, name? }
 *   config: { apiUrl, courseId, moodleUserId, courseName? }
 */
(function (global) {
    'use strict';
    ...
    function loadScriptNoAmd(src) {
        return new Promise(function (resolve, reject) {
            ...
            var defineBackup = global.define;
    ...
    function ensurePdfMake() {
        if (global.pdfMake && typeof global.pdfMake.createPdf === 'function') {
            return Promise.resolve(global.pdfMake);
        }
    ...
    function buildPdfDocDefinition(exportData, fallbackCourseName) {
            ...
            content: []
                .concat(
                    buildPdfHeaderContent(
                        kind,
                        exportData.name || '',
                        exportData.courseName || fallbackCourseName || ''
                    )
                )
                .concat(body)
    ...
    function downloadAiContentPdf(exportData, fallbackCourseName) {
        return ensurePdfMake().then(function (pdfMake) {
            var doc = buildPdfDocDefinition(exportData, fallbackCourseName);
    ...
    function fetchExport(item, config) {
        var apiUrl = String(config.apiUrl || '').replace(/\/$/, '');
        var path =
            '/ai-content/export?courseId=' + encodeURIComponent(config.courseId) +
            '&moodleUserId=' + encodeURIComponent(config.moodleUserId) +
    ...
    function download(item, config) {
        ...
        if (!config || !config.apiUrl || !(config.courseId > 1) || !(config.moodleUserId > 0)) {
            return Promise.reject(new Error('Missing PDF export config'));
        }

        return fetchExport(item, config).then(function (data) {
            ...
            return downloadAiContentPdf(data, config.courseName || '');
        });
    }

    global.SyllentrasAiContentPdf = {
        download: download
    };
